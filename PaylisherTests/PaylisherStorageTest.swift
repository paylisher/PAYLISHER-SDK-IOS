...
class PaylisherStorageTest: QuickSpec {
    func getSut() -> PaylisherStorage {
        let config = PaylisherConfig(apiKey: "123")
        return PaylisherStorage(config)
    }
    ...
    override func spec() {
        ...
        it("falls back to application support directory when app group identifier is not provided") {
            ...
            let sut = PaylisherStorage(config)
